...
| `config` | Provider overrides. For Salesforce: describe JSON that replaces baked metadata. Omit to keep defaults. See [Salesforce](/providers/salesforce). |
...
Exact fields inside each collection depend on the provider. For Salesforce seed and config templates, see [Salesforce](/providers/salesforce).
